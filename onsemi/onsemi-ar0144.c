					     ...
					     struct onsemi_limits *limits))
{
	...
	/* developer guide "Blanking Control" */
	limits->hblank_min = 208;

	limits->pll_op_sys_clk_div = (struct onsemi_range) {
		/* datasheet is wrong; op_sys_clk_div is *not* read-only */
		.min	= 1,
		.max	= 2,
	};
				      ...
				      unsigned int v, s32 *res)
{
	...
	/* eq.42 - 44 in "Developer guide":
	 *
	 *    fine = 1 / (1 - t  /32)
	 *
	 * Set t = 15.5 (short above fine max) and transform this equation a
	 * little bit and scale by 1000 to find out the 'coarse' value.
	 */
	...
	/* eq.45
	 *
	 *    total gain = 2^coarse * fine
	 *
	 * Check short-below min case first and calculate 't' (see eq. 42)
	 * accordingly this equation else
	 */
	if (v <= (1u << coarse) * 1000) {
		fine = 0;
	} else {
...
static int ar0144_prepare(struct onsemi_core *sensor)
{
	...
	if (sensor->active_bus && sensor->active_bus->bus_type == V4L2_MBUS_CSI2_DPHY) {
		...
		/* TODO: this is ugly and must be solved in another way
		 * (e.g. in link_setup) */
		onsemi_update_bits(&rc, sensor, 0x301a, BIT(2), BIT(2)); /* enable streaming */
	}
...
static struct regmap_range const		ar0144_regmap_octet_table_yes[] = {
	{ 0x301c, 0x301d },		/* mode_select, image_orientation */
	{ 0x3021, 0x3024 },		/* sofware_reset - pixel_order */
};
...
static struct regmap_range const		ar0144_regmap_wr_table_no[] = {
	/* the 8 bit address ranges */
	{ 0x301c, 0x301d },		/* mode_select, image_orientation */
	{ 0x3021, 0x3024 },		/* sofware_reset - pixel_order */

	{ 0x3026, 0x3026 },		/* gpi_status */
	{ 0x303a, 0x303c },		/* frame_count + frame_status */

	{ ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3024), /* pixel_order */
...
static struct regmap_range const		ar0144_regmap_rd_table_yes[] = {
	...
	{ ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3021),
	  /* software_reset, group_hold, mask_corrupted_frames, pixel_order */
	  ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3024) },
};
...
static struct regmap_range const		ar0144_regmap_vol_table_yes[] = {
	{ 0x301a, 0x301a },	/* reset_register */
	{ 0x3026, 0x3026 },	/* gpi_status */
	{ 0x303a, 0x303c },		/* frame_count + frame_status */
	{ 0x312a, 0x312a },		/* ae_current_gains */
	{ 0x3150, 0x3164 },		/* ae histogram values */
	{ ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3021), /* software_reset */
	  ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3021) },
	{ ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3024), /* pixel_order */
	  ONSEMI_REGMAP_8BIT_REG_TO_ADDR(0x3024) },
};
