...
enum onsemi_pwr_state {
	/* all VDD/VAA off, reset asserted, clock disabled */
	ONSEMI_PWR_OFF,
	/* all VDD/VAA on, reset deasserted, clock enabled */
	ONSEMI_PWR_ON,
	/* all VDD/VAA on, reset asserted, clock enabled */
	ONSEMI_PWR_RST,
};
...
struct onsemi_v4l_parm {
	/* can be modified only, when streaming has been stopped */
	unsigned int		code;
	unsigned int		bpp;
...
struct onsemi_core_ops {
	/* sensors support SMIA configuration range */
	bool		has_smia_cfg:1;
	bool		hclk_mul_2:1;

	/* bitmaks of supported bpp */
	unsigned long	supported_bpp;

	/* apply VAA/VAAPIX before VDDIO */
	bool		vaa_first:1;
...
struct onsemi_dev_cfg {
	...
	/* -1 -> autodetect based on customer rev 0x31fe register */
	int					is_color;
