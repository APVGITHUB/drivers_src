...
static int _onsemi_power_reset(struct onsemi_core *sensor, bool do_assert)
{
	...
	if (do_assert) {
		 ...
		 * explicit STOP state */
		usleep_range(10, 10000); /* t_101 */
	} else {
		msleep(100);		/* t_5 = 45000 EXTCLKs */
...
static int _onsemi_power_on(struct onsemi_core *sensor)
{
	...
	/* see Figure 15 "Power-Up Sequence" and Table 16 "Power-Up Sequence"
	 * for details */
					...
					enum onsemi_pwr_state state)
{
	...
	static struct onsemi_state_transition const	STATES[3][3] = {
		...
		/* TODO: this violates "Figure 16 "Power-Down sequence which
		 * has an extra STOP state where clock is disabled and RESET
		 * deasserted */
		[ONSEMI_PWR_ON] = {
			[ONSEMI_PWR_OFF]	= {
...
static int onsemi_core_get_temp(void *onsemi_, int *res)
{
	...
	if ((ctrl_reg & BIT(0)) != 0) {
		...
		/* clear conversion + start bits */
		ctrl_reg &= ~(BIT(5) | BIT(4));

		/* power thermal sensor on and clear old value */
	...
	/* first multiplication implies '* 1000 / 1000' scaling */
...
static int onsemi_core_thermal_init(struct onsemi_core *onsemi)
{
	...
	if (rc == -ENODEV) {
		/* not configured in dtree; ignore it */
	...
	t0  = t_ref;
	t0 -= cal * slope;	/* implies '* 1000 / 1000' scaling */
			...
			struct onsemi_dev_cfg const *cfg)
{
	...
	if (cfg->ops->has_smia_cfg) {
		...
		ver.chip = onsemi_read(&rc, onsemi, 0x3000); /* model_id */
				 ...
				 bool do_wait, int *err)
{
	...
	/* check whether serial mode was enabled */
	...
	while (rc >= 0) {
		...
		if (is_first) {
			/* when called the first time, ensure that standby
			 * mode has been activated.  Disable GPIO input to
			 * avoid staying alive because of asserted TRIGGER */
	...
	if (is_smia) {
		/* wait one frame before disabling MIPI interface */
...
static void onsemi_params_write_pll(struct onsemi_core *onsemi, int *err)
{
	...
	/* TODO: add a way to force LSB alignung; e.g. AR0144's 0x3736[5]
	 * bit? */
...
static int _onsemi_s_stream_on(struct onsemi_core *onsemi)
{
	...
	if (!IS_ENABLED(CONFIG_THERMAL)) {
		/* enable thermal sensor and clear previous results (for
		 * sanity) */
...
static int _onsemi_s_stream_off(struct onsemi_core *onsemi)
{
	...
	/* TODO: this is hacky but required for LP-11 required on iMX6 */
	if (rc == 0)
...
again:
	...
	/* TODO: some OnSemi sensors might support other values... */
				   ...
				   struct v4l2_subdev_frame_interval *interval)
{
	...
	/* calculate a matching 'vblank' value with an epsilon which can be
	 * compensated by EXTRA_DELAY (not implemented yet!) */
				 ...
				 struct onsemi_v4l_parm *parm)
{
	...
	rc = 0;
	y[0] = onsemi_read(&rc, onsemi, 0x3002); /* x_addr_start */
	x[0] = onsemi_read(&rc, onsemi, 0x3004); /* y_addr_start */
	y[1] = onsemi_read(&rc, onsemi, 0x3006); /* x_addr_end */
	x[1] = onsemi_read(&rc, onsemi, 0x3008); /* y_addr_end */
